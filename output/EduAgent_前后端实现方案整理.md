# EduAgent 项目前后端实现方案整理

_生成时间：2026-07-10 19:59:04；项目路径：C:\Users\17873\Desktop\EduAgent_

本文档基于当前工作区源码自动梳理并人工结构化整理，覆盖 EduAgent 项目的产品定位、总体架构、后端实现、前端实现、核心业务流程、API 契约、部署测试以及后续优化建议。文档重点面向技术评审、答辩汇报、团队交接与后续迭代开发。

| 项目 | 内容 |
| --- | --- |
| 项目名称 | EduAgent：基于大模型的个性化资源生成与学习多智能体系统 |
| 代码根目录 | C:\Users\17873\Desktop\EduAgent |
...
| 输出文档 | C:\Users\17873\Desktop\EduAgent\output\EduAgent_前后端实现方案整理.docx |
...
EduAgent 是一个面向学习者的智能教育系统。它以知识图谱为学习地图，以学习者画像为个性化依据，以多智能体协同为核心决策机制，动态完成冷启动画像、学习路径规划、知识点资源生成、Tutor 答疑、行为评估、掌握度更新、重规划与能力诊断。前端提供课程选择、画像问答、知识路径、资源画布、导师对话、能力雷达和诊断反馈等交互，后端负责会话状态、算法编排、资源生成、鉴权、持久化与观测。

### 1.2 代码规模

| 模块 | 文件数 | 约代码行数 | 说明 |
| --- | --- | --- | --- |
| src | 109 | 21920 | 后端核心包：Agent、应用服务、数据库、图谱、向量、LLM、领域模型等 |
| frontend/src | 59 | 13193 | Vue 3 前端源码：页面、组件、组合式状态、API 客户端、样式 |
| tests | 42 | 8950 | 单元测试与集成测试，覆盖算法节点、应用服务、鉴权和 HTTP 行为 |
...
| langchain-core | langchain-core>=0.3.0,<1.0.0 |
| neo4j | neo4j>=5.20.0,<6.0.0 |
| pymilvus | pymilvus>=2.3.0,<3.0.0 |
| elasticsearch | elasticsearch>=8.17.0,<9.0.0 |
| pydantic | pydantic>=2.5.0,<3.0.0 |
| numpy | numpy>=1.24.0,<2.0.0 |
| langchain-text-splitters | langchain-text-splitters>=0.3.0,<1.0.0 |
| pytest | pytest>=7.4.0,<9.0.0 |
| pytest-asyncio | pytest-asyncio>=0.21.0,<1.0.0 |
| aiohttp | aiohttp>=3.9.0,<4.0.0 |
| sentence-transformers | sentence-transformers>=3.0.0,<6.0.0 |
| openai | openai>=1.50.0 |
| httpx | httpx>=0.27.0 |
| sse-starlette | sse-starlette>=1.8.0,<2.0.0 |
| starlette | starlette>=0.36.0,<1.0.0 |
| fastapi | fastapi>=0.104.0,<1.0.0 |
| uvicorn | uvicorn>=0.24.0,<1.0.0 |
...
| sqlalchemy | sqlalchemy[asyncio]>=2.0.35,<3.0.0 |
...
| --- | --- | --- |
| axios | ^1.9.0 | runtime |
| dompurify | ^3.2.6 | runtime |
| marked | ^16.0.0 | runtime |
| mermaid | ^11.12.0 | runtime |
| vue | ^3.5.18 | runtime |
| vue-router | ^4.6.4 | runtime |
...
项目采用前后端分离但同容器交付的模式：Vite 构建后的前端静态资源由 Starlette 服务托管；同一个 Starlette 应用暴露业务 API、鉴权 API、兼容 API 与观测 API。后端内部按入口层、应用服务层、编排层、智能体层、领域状态层、数据基础设施层、外部模型/检索层分层。
...
1. 访问首页，进入应用工作台；若无有效登录态则进入登录/注册流程。
2. 登录成功后拉取用户信息、已选课程、可选课程；没有当前课程时进入课程选择。
3. 创建或恢复学习 session，若画像未完成则进入 profile probe 冷启动问答。
4. 画像完成后初始化学习路径，前端进入 ready 模式并加载当前知识点资源。
5. 用户阅读资源、完成练习、向 Tutor 提问，前端将行为数据提交给后端。
6. 后端运行官方学习步骤：Evaluator、Profiler、Planner、Tutor、ContentMesh、Validator、Assessment 协同更新状态。
7. 前端接收 SessionResponse，刷新路径、资源、Tutor 消息、Agent 时间线、能力雷达和诊断报告。
8. 当掌握度达到阈值时推进到下一个知识点；当 PID 或策略触发时重规划路径或调整教学策略。
...
当前服务入口位于 frontend/server.py。它不是单纯的前端目录文件，而是生产运行时的 Starlette 应用入口：注册 REST API、SSE 流式接口、鉴权接口、旧版兼容接口，并在构建产物存在时托管 frontend/dist 静态文件。Dockerfile 最终命令也是 python frontend/server.py。

| 方法 | 路径 | 处理函数 |
| --- | --- | --- |
| POST | /api/sessions | api_create_session |
| GET | /api/sessions/{session_id} | api_get_session |
| GET | /api/sessions/{session_id}/profile-probe | api_session_profile_probe |
| POST | /api/sessions/{session_id}/profile-input | api_session_profile_input |
| POST | /api/sessions/{session_id}/path/init | api_session_init_path |
| POST | /api/sessions/{session_id}/advance | api_session_advance |
| POST | /api/sessions/{session_id}/behavior | api_session_behavior |
| POST | /api/sessions/{session_id}/tutor | api_session_tutor |
| POST | /api/sessions/{session_id}/tutor-stream | api_session_tutor_stream |
| POST | /api/sessions/{session_id}/replan | api_session_replan |
| GET | /api/sessions/{session_id}/resources/{node_id} | api_session_resources |
| GET | /api/courses | api_list_courses |
| GET | /api/courses/{course_id} | api_get_course |
| GET | /api/user/courses | api_get_user_courses |
| POST | /api/user/courses/enroll | api_enroll_course |
| POST | /api/user/courses/switch | api_switch_course |
| GET | /api/knowledge-graph | api_knowledge_graph |
| GET | /api/ops/metrics | api_ops_metrics |
| POST | /api/reset | api_reset |
| GET | /api/state | api_compat_get_state |
| GET | /api/cold-start/probe | api_compat_cold_start_probe |
| POST | /api/cold-start/answer | api_compat_cold_start_answer |
| POST | /api/init-path | api_compat_init_path |
| POST | /api/pipeline/step | api_compat_run_pipeline_step |
| GET | /api/pipeline/stream | api_compat_stream_pipeline |
| POST | /api/tutor/ask | api_compat_ask_tutor |
| POST | /api/tutor/ask-stream | api_compat_ask_tutor_stream |
| POST | /api/resources/generate-node | api_compat_generate_node_resources |
| GET | /api/auth/captcha | api_auth_captcha |
| GET | /api/auth/captcha-json | api_auth_captcha_json |
| POST | /api/auth/register | api_auth_register |
| POST | /api/auth/login | api_auth_login |
| POST | /api/auth/refresh | api_auth_refresh |
...
run_official_learning_step 是当前后端最关键的学习闭环入口。它将一次用户交互转化为可追踪的 Agent 流水线，确保状态更新、资源生成、Tutor 和评测在统一语义下执行。

1. 解析输入：确定 current_node、correctness、time_spent_ratio、code_pass_rate、help_count、tutor_query 与 interaction_type。
2. Evaluator：在非 load_node 场景下清洗行为向量，识别异常和错误类型，计算 PID 控制量并决定是否触发重规划。
3. Profiler：更新动态画像，包括知识掌握度、错误分布、连续失败计数、遗忘曲线相关记录。
4. Planner：当 active_path 为空或重规划标记为真时，结合知识图谱和掌握度重新计算学习路径。
5. Tutor：当存在 tutor_query 且不是单纯加载节点时，调用 TutorAgentNode 生成个性化答疑。
6. ContentMesh/ResourceService：为当前节点生成或读取资源卡片，并处理预生成与缓存。
7. Validator：对资源进行符号/AST 与 NLI 层面的反幻觉校验，校验结果写入资源元数据和 Agent 反馈。
8. Assessment：生成能力雷达、策略控制和诊断报告，更新 dynamic_profile。
9. 推进节点：在诊断或测验达到掌握阈值后切换 current_node，维持 active_path 状态。
10. 返回 LearningStepResult：最终由应用服务转换为前端需要的 SessionResponse。

    run_official_learning_step(input, runtime_session)
      -> evaluator_output
...
### 3.5 OrchestrationRuntime 运行时

OrchestrationRuntime 是稳定的官方运行时单例，集中管理 LLM Provider、知识图谱管理器、各类 Agent 节点和 session runtime。这样可以避免 HTTP 请求中反复构造模型客户端和图谱连接，也使测试可以替换 session state 或重置 runtime。
...
### 3.7 AgentState 与核心领域状态

AgentState 是多智能体协作的共享状态。它将用户、课程、静态画像、动态画像、最新行为、学习路径、资源、反馈和诊断结果组织在同一个强类型 Pydantic 模型中，便于在 Agent 节点之间安全传递并最终序列化给前端。
...
当前 agent_state.py 中识别到的主要类包括：CognitiveStyleDistribution、StaticProfile、KnowledgeMasteryRecord、ErrorTypeDistribution、PIDErrorRecord、DynamicProfile、LatestBehavior、ResourceCard、AgentFeedbackItem、AgentState。

### 3.8 数据持久化与仓库层

数据库层使用 SQLAlchemy asyncio 建模，按 Repository 拆分用户、课程、选课、会话、状态、资源、评测等数据访问。开发或测试场景可使用 SQLite/fakeredis，生产 compose 中预留 PostgreSQL、Redis 等服务。
...
models_sqlalchemy.py 中识别到的类包括：Base、ResourceType、DifficultyLevel、LearningStage、TaskStatus、User、StudentProfile、LearningSessionModel、SessionSnapshotModel、LearningPath、LearningTask、GeneratedResource、ConversationHistory、QuizRecord、EvaluationReport、KnowledgePoint、AgentTaskLog。
...
### 4.3 useEduAgent 组合式状态

useEduAgent.js 是前端事实上的状态中枢。它将 API 调用、业务状态、计算属性、用户动作和界面反馈组织为一个组合式 hook，供 AppWorkspaceView 和子组件使用。
...
eduAgentApi.js 中识别到的导出函数包括：getCaptcha、register、login、refreshToken、fetchMyProfile、buildSessionId、createSession、getSession、submitSessionProfileInput、fetchSessionProfileProbe、initSessionPath、advanceSession、submitSessionBehavior、askSessionTutor、streamSessionTutor、replanSession、fetchSessionResources、resetSession、fetchKnowledgeGraph、fetchCourses、fetchCourseDetail、fetchUserCourses、enrollCourse、switchCourse。
...
前端围绕 ResourceCard 进行渲染。后端必须保证每张卡片具备稳定的 resource_id、node_id、card_type、title/content/status/metadata。content 通常为 Markdown，metadata 承载难度、校验状态、渲染类型、题目答案、代码语言等扩展字段。

| card_type | 前端渲染倾向 | 典型内容 |
| --- | --- | --- |
...
1. 前端创建 session 后调用 profile-probe 获取下一道画像问题。
2. 用户在 ChatArea 中作答，前端调用 profile-input 提交答案。
3. ProfileService 将答案交给 ColdStartOrchestrator，更新 StaticProfile 的认知风格、动机、基础知识、时间预算等字段。
4. 当 is_complete 为真时 finalize 画像，确定 target_node 或初始学习目标。
5. 前端从 probe 模式切换到路径初始化，随后进入 ready。

### 5.2 节点学习与资源生成流程

1. 前端根据 currentNode 请求 /api/sessions/{session_id}/resources/{node_id} 或在 advance/behavior 响应中获取资源。
2. ResourceService 检查资源缓存；缺失或过期时调用 ContentMeshNode 创建生成任务。
3. ContentMeshNode 使用 WFQ 调度不同资源类型，保障测验、概念解释、代码练习等资源公平生成。
4. ResourceGenerationAgent 根据课程、节点、画像和资源类型生成 Markdown/结构化内容。
5. ValidatorNode 对内容进行规则校验、AST 校验或 NLI 校验，写入 status 和 metadata。
6. 前端 ResourceCanvas 按 card_type 渲染，MarkdownContent 对 HTML 进行 DOMPurify 净化并渲染 Mermaid。

### 5.3 测验推进与重规划闭环

1. 用户完成测验或练习后，前端整理 correctness、accuracy_rate、code_pass_rate、duration_ratio、help_request_count 等行为数据。
2. EvaluatorNode 清洗行为并识别异常，例如耗时异常、连续错误、求助过多、掌握度偏离。
3. PIDReplanController 根据误差累计和变化趋势判断是否需要重规划。
4. ProfilerNode 更新对应 node_id 的 knowledge_mastery，并记录错误类型分布和能力信号。
5. 如果掌握度达到阈值，SessionService 推进到 activePath 的下一个节点；如果触发重规划，则 PlannerNode 重新计算路径。
6. AssessmentReporterNode 更新能力雷达和诊断报告，前端同步刷新进度、知识树和建议。
...
### 6.2 SessionResponse 结构

SessionResponse 是前端最核心的响应契约，目标是让一次后端调用即可刷新工作台所需大部分状态。它通常包含 session、profile、dynamic_profile、learning_path、resources、assessment、tutor_response、agent_feedback、pipeline_log 等字段，并带有 resource_contract_version 与 agent_feedback_version 以支持前后端协同演进。
...
Tutor 流式接口适合长回答或逐步解释。后端使用 sse-starlette/Starlette 流式能力分片输出，前端 streamSessionTutor 将 chunk 追加到当前 assistant 消息。实现上需要保证异常 chunk、结束事件、Token 过期重试和用户取消的处理。

## 7. 部署、运行、观测与测试

### 7.1 Docker 部署

Dockerfile 采用多阶段构建：第一阶段使用 Node 构建 Vue 前端，第二阶段使用 python:3.11-slim 安装 requirements，复制 src、tests、scripts、frontend/server.py 和前端 dist，暴露 8800 端口，健康检查调用 /api/state，启动命令为 python frontend/server.py。

| docker-compose 服务 | 用途 |
| --- | --- |
...
tests 目录覆盖了核心算法、Agent 节点、应用服务、鉴权、HTTP 可观测性和前端资源契约。建议在提交前执行 pytest，并在需要时补充端到端用例。
...
1. 本地开发前端：进入 frontend 后执行 npm install 与 npm run dev，API 走代理或同源配置。
2. 本地后端：安装 requirements 后执行 python frontend/server.py，默认服务端口以 server.py 配置为准，Docker 中暴露 8800。
3. 完整依赖：使用 docker-compose up 启动 app、Neo4j、Elasticsearch、Milvus、MinIO、etcd。
4. 测试：使用 pytest 运行 tests；对 LLM 相关测试建议使用 mock 或测试 Provider，避免真实费用和不稳定。
5. 观测：通过 X-Request-ID 定位一次请求，在 /api/ops/metrics 查看指标快照，在日志中查看 Agent 阶段事件。
...
从当前源码看，EduAgent 已经形成较完整的智能教育系统原型：前端具备完整学习工作台体验，后端具备多智能体学习闭环、资源生成、路径规划、鉴权、持久化、图谱和检索能力。后续最值得优先投入的是安全清理、接口契约稳定化、异步资源生成、端到端测试和运行观测体系完善。

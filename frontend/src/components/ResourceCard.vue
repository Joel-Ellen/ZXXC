<template>
  <article
    class="resource-card relative w-full overflow-hidden transition-all duration-300"
    :class="[
      compact ? 'p-0' : 'rounded-[20px] p-6',
      !isReady
        ? (compact ? 'opacity-80' : 'border border-subtle bg-space-panel opacity-80')
        : (compact ? 'bg-transparent' : `${colorClasses.border} ${colorClasses.hoverBorder} bg-space-panel`),
  ...
  >
    <div class="mb-5 flex items-start justify-between gap-3">
      ...
      <div class="flex shrink-0 items-start gap-2">
        ...
        <button
          v-if="showPin"
          type="button"
          class="focus-ring rounded-full border border-subtle p-2 transition-all duration-200 hover:border-primary/30 hover:bg-card-hover hover:text-primary active:scale-95"
          :class="isPinned ? 'border-primary/35 bg-primary-soft text-primary' : 'text-text-muted'"
          :aria-pressed="String(isPinned)"
          :aria-label="isPinned ? '取消置顶卡片' : '置顶卡片'"
          :title="isPinned ? '取消置顶卡片' : '置顶卡片'"
          @click.stop="$emit('pin')"
          @keydown.enter.stop
          @keydown.space.stop
        >
    ...
    <div class="mb-5 h-px w-full bg-[var(--border-subtle)]" />
    ...
    <div class="relative min-h-[120px]">
      <div v-if="!isReady" class="absolute inset-0 space-y-3 overflow-hidden rounded-2xl">
        <div
          ...
          :class="width"
        >
          <div class="h-full w-1/2 bg-[var(--border-subtle)]" />
        </div>
    ...
    <div v-if="!compact" class="mt-6 flex flex-wrap items-center justify-between gap-2">
...
const footerLabel = computed(() => (
  props.isReady
    ? (props.isPinned ? "已置顶" : "资源就绪")
    : "资源仍在装配中，建议暂时停留在当前节点等待生成完成。"
));
function handleActivate(event) {
  if (!props.activatable || isInteractiveTarget(event)) {
    return;
  }

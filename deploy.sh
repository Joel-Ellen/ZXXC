...
setup_nginx_hint() {
    ...
    log_info "=========================================="
    echo ""
    echo "  生产模板: nginx/edu-agent.conf（已启用，无需取消注释）"
    echo ""
    echo "  生产反向代理:"
    echo "    1. 编辑 nginx/edu-agent.conf，将 server_name 改为你的域名"
    echo "    2. sudo cp nginx/edu-agent.conf /etc/nginx/sites-available/edu-agent"
    echo "    3. sudo ln -s /etc/nginx/sites-available/edu-agent /etc/nginx/sites-enabled/"
    echo "    4. sudo nginx -t && sudo nginx -s reload"
    echo ""
    echo "  本地验证或第二监听器:"
    echo "    nginx/edu-agent-standalone.conf 在 8080 端口提供反向代理，"
    echo "    应用仍监听 127.0.0.1:8800；启用前请运行 sudo nginx -t。"
    echo ""
    log_info "=========================================="
    echo ""
}

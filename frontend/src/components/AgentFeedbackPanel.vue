  ...
  <section class="flex min-h-0 flex-col gap-4">
    <div class="workspace-shell-card rounded-2xl px-4 py-4">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
          <p class="text-[10px] font-black uppercase tracking-[0.14em] text-text-muted">Agent 反馈</p>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              <span class="text-[10px] font-black uppercase tracking-[0.14em] text-text-muted">{{ item.stage || item.agent }}</span>
              <span
            ...
            <p class="mt-2 text-sm font-semibold text-text-primary">{{ item.headline || item.agent }}</p>
    ...
    <div v-else class="workspace-shell-card rounded-2xl px-4 py-4">
      <p class="text-sm leading-6 text-text-muted">
        当前还没有可展示的结构化反馈。完成一次节点加载、诊断或辅导后，这里会同步显示各 agent 的完整输出。
      </p>
    </div>
  </section>
</template>

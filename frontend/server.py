...
def get_or_create_session(user_id: str) -> Dict[str, Any]:
    if user_id not in sessions:
        ...
        def _fallback_generate(node_id: str, card_type: str, difficulty: float) -> str:
            """无 LLM 时的纯知识库 / 模板生成。"""
            ...
            def _hybrid_generate(node_id: str, card_type: str, difficulty: float) -> str:
                if card_type == "concept_map":
                    try:
                        ...
                        pass
                return _fallback_generate(node_id, card_type, difficulty)
            mesh = ContentMeshNode(generate_fn=_hybrid_generate)
        else:
            mesh = ContentMeshNode(generate_fn=_fallback_generate)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    # 后台预加载 ES 知识库（模型下载可能耗时较长，不阻塞服务启动）
    def _warmup_es_kb():
        print("[Init] Loading knowledge base (background)...")
        _safe_get_es_kb()
        print("[Init] Knowledge base ready.")
    threading.Thread(target=_warmup_es_kb, daemon=True).start()
    print("[Init] Server starting (KB loading in background)...")

<template>
  <div class="min-h-screen text-text-primary font-sans relative">
    ...
    <LandingView
      v-if="showLanding"
      :is-logged-in="hasStoredToken"
      @enter="onEnterApp"
      @logout="onLandingLogout"
    />
...
const hasStoredToken = ref(!!window.localStorage.getItem("access_token"));
...
function goHome() {
  ...
  hasStoredToken.value = !!window.localStorage.getItem("access_token");
}

function onLandingLogout() {
